...
def greedy_model(env):
    ...
    for episode in range(max_episodes):
        ...
        for steps in range(num_steps):
            ...
            random_slot = steps
            # print('time period', random_slot)

            env.traffic_load = actual_traffic[random_slot]
            # print('traffic load', env.traffic_load)

            # Convert the NumPy array to a PyTorch tensor
            demand_data = torch.from_numpy(env.traffic_load)

            # Get the next locations to serve based on demand threshold
            next_locations = torch.where(demand_data > 0)

            # Sort the next locations based on demand in descending order
            sorted_indices = torch.argsort(demand_data[next_locations], descending=True)

            # Initialize the drone locations randomly
            indices = sorted_indices[:env.num_base_stations]

            action = indices.detach().numpy()
            # print('Action', action)
    ...
    filename = 'evaluation_results_01.01_GreedyModel.csv'
